...
const STATUS_CONFIG: Record<PlatformConnection['status'], { label: string; icon: React.ReactNode; className: string; dot: string }> = {
  ...
  disconnected: { label: '未连接', icon: <Link2 className="h-3 w-3" />, className: 'bg-slate-800 text-slate-400', dot: 'bg-slate-1000' },
    ...
    <div className="flex h-full flex-col gap-4 rounded-2xl border border-slate-200 bg-[rgba(15,23,42,0.85)] p-5 backdrop-blur-xl">
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          disabled={syncing}
          className="flex items-center gap-1.5 rounded-lg bg-slate-100 px-3 py-1.5 text-xs font-medium text-slate-400 hover:bg-[rgba(255,255,255,0.1)] hover:text-slate-900 disabled:opacity-50"
        ...
        <div className="flex flex-1 flex-col gap-2 overflow-y-auto">
          ...
          {platforms.map((platform) => {
            ...
            return (
              ...
              >
                {/* Platform Icon */}
                <div className={`flex h-9 w-9 items-center justify-center rounded-lg bg-slate-100 ${PLATFORM_COLORS[platform.platform]}`}>
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    <p className="text-sm font-medium text-slate-900 truncate" style={{ fontFamily: 'monospace' }}>
                ...
                <div className="flex items-center gap-2">
                  ...
                  {platform.autoPublish && (
                    <span className="rounded bg-[#2B59C3]/30 px-1.5 py-0.5 text-[10px] text-blue-600">
                      自动
                    </span>
                  )}
                  ...
                  {platform.status === 'connected' && (
                    ...
                    >
                      <RefreshCw className={`h-3.5 w-3.5 ${isSyncingThis ? 'animate-spin text-emerald-600' : ''}`} />
                    </button>
                  )}
        ...
        <div className="w-72 flex-shrink-0 overflow-y-auto">
          ...
          <div className="space-y-2">
            {scheduled.map((post) => (
              ...
              >
                ...
                <p className="mt-1.5 truncate text-xs text-slate-900" style={{ fontFamily: 'monospace' }}>
                  {post.title}
                </p>
          ...
          {selectedPlatform && (
            <div className="mt-4 rounded-xl border border-slate-200 bg-slate-100 p-4">
              <h3 className="text-xs font-semibold text-slate-900" style={{ fontFamily: 'monospace' }}>
              ...
              <div className="mt-4 flex flex-col gap-1.5">
                <button className="flex items-center justify-center gap-1.5 rounded-lg bg-[#14D1A0]/20 px-3 py-2 text-xs font-medium text-emerald-600 hover:bg-[#14D1A0]/30">
                  <Zap className="h-3 w-3" /> 配置自动发布
                </button>
                <button className="flex items-center justify-center gap-1.5 rounded-lg bg-slate-100 px-3 py-2 text-xs font-medium text-slate-400 hover:bg-[rgba(255,255,255,0.1)] hover:text-slate-900">
