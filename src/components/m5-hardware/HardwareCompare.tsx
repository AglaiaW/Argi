      ...
      <table className="w-full text-xs">
        <thead>
          ...
          <tr className="border-b border-white/10">
            <th className="text-left p-4 bg-white/[0.03] sticky left-0 z-10 min-w-[180px]">
              <span className="text-[10px] text-white/40 uppercase tracking-wider font-medium">Hardware</span>
            </th>
            {items.map(item => (
              <th key={item.id} className="p-4 bg-white/[0.03] min-w-[200px] text-center">
                <div className="flex flex-col items-center gap-2">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-white/90">{item.vendor}</p>
                    <p className="text-[10px] text-white/40 mt-0.5">{item.modelName}</p>
                  </div>
                  <SellerLevelBadge level={item.level} size="sm" />
                  <div className="text-base font-bold text-emerald-400">${item.pricePerUnit}<span className="text-[10px] text-white/40 font-normal">/{item.priceUnit}</span></div>
                  <div className="flex items-center gap-2 mt-1">
                    <span className="text-[10px] text-white/40">★ {item.rating}</span>
                    <span className="text-[10px] text-white/40">{item.reviews.toLocaleString()} reviews</span>
                  </div>
        ...
        <tbody>
          ...
          <tr className="border-b border-white/5">
            <td className="p-3 pl-4 text-white/40 sticky left-0 bg-[#010409]">Lead Time</td>
            {items.map(i => <td key={i.id} className="p-3 text-center text-white/60">{i.leadTime}</td>)}
          </tr>
          <tr className="border-b border-white/5">
            <td className="p-3 pl-4 text-white/40 sticky left-0 bg-[#010409]">Location</td>
            {items.map(i => <td key={i.id} className="p-3 text-center text-white/60">{i.location}</td>)}
          </tr>
          {items[0]?.utilizationRate !== undefined && (
            <tr className="border-b border-white/5">
              <td className="p-3 pl-4 text-white/40 sticky left-0 bg-[#010409]">Utilization</td>
              {items.map(i => (
                <td key={i.id} className="p-3 text-center">
                  <div className="flex items-center justify-center gap-2">
                    ...
                    <span className="text-white/60 tabular-nums">{i.utilizationRate}%</span>
                  </div>
                </td>
              ))}
            </tr>
          ...
          {allSpecLabels.map(label => (
            <tr key={label} className="border-b border-white/5 hover:bg-white/[0.02] transition-colors">
              <td className="p-3 pl-4 text-white/40 sticky left-0 bg-[#010409]">{label}</td>
              {items.map(i => (
          ...
          {/* Certifications */}
          <tr className="border-b border-white/5">
            <td className="p-3 pl-4 text-white/40 sticky left-0 bg-[#010409]">Certifications</td>
            {items.map(i => (
              <td key={i.id} className="p-3 text-center">
          ...
          {/* Highlights */}
          <tr className="border-b border-white/5">
            <td className="p-3 pl-4 text-white/40 sticky left-0 bg-[#010409]">Highlights</td>
            {items.map(i => (
              <td key={i.id} className="p-3 text-center">
                <div className="flex flex-wrap gap-1 justify-center">
                  {i.highlights.slice(0, 3).map(h => (
                    <span key={h} className="inline-flex items-center rounded-full bg-white/5 border border-white/10 px-2 py-0.5 text-[9px] text-white/50">{h}</span>
                  ))}
                </div>
              </td>
            ))}
  ...
  return (
    <div className="flex items-center gap-3 rounded-xl bg-white/[0.03] border border-white/10 p-3 hover:bg-white/[0.05] transition-colors">
      <span className={[
        ...
        rank === 2 ? 'bg-white/10 text-white/60 border border-white/20' :
      ...
      <div className="flex-1 min-w-0">
        <p className="text-sm font-medium text-white/80 truncate">{item.vendor}</p>
        <p className="text-[10px] text-white/40 truncate">{item.modelName}</p>
      </div>
      ...
      <div className="text-right shrink-0">
        <p className="text-sm font-medium text-white/70">★ {item.rating}</p>
        <p className="text-[10px] text-white/40">{item.reviews.toLocaleString()}</p>
      </div>
    </div>
  )
}
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-2">
          <h2 className="text-base font-semibold text-white/90">Hardware Comparison</h2>
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <div className="flex items-center rounded-xl bg-white/[0.04] border border-white/10 overflow-hidden">
            <button onClick={() => setActiveTab('side_by_side')} className={['px-3 py-2 text-xs font-medium transition-colors', activeTab === 'side_by_side' ? 'bg-white/15 text-white/80' : 'text-white/40 hover:text-white/60'].join(' ')}>
              <BarChart3 className="h-4 w-4 inline mr-1" />Side by Side
            </button>
            <button onClick={() => setActiveTab('ranking')} className={['px-3 py-2 text-xs font-medium transition-colors', activeTab === 'ranking' ? 'bg-white/15 text-white/80' : 'text-white/40 hover:text-white/60'].join(' ')}>
          ...
          <button
            ...
            className="flex items-center gap-1.5 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-xs text-white/60 px-3 py-2 transition-colors disabled:opacity-40"
          >
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <span className="text-[10px] text-white/40">Quick compare:</span>
        {PRESET_COMPARES.map(preset => (
          <button
            ...
            className="inline-flex items-center gap-1 rounded-lg bg-white/5 hover:bg-white/10 border border-white/10 text-[10px] text-white/50 px-2.5 py-1.5 transition-colors"
          >
            {preset.label}
          </button>
        ))}
        ...
        <div className="space-y-4">
          {compareItems.length < 2 && (
            <div className="text-center py-12 rounded-2xl bg-white/[0.02] border border-white/10">
              <BarChart3 className="h-8 w-8 text-white/20 mx-auto mb-2" />
              <p className="text-sm text-white/40">Select at least 2 items to compare</p>
          ...
          {compareItems.length > 0 && (
            <div className="flex items-center gap-2 flex-wrap">
              {compareItems.map(item => (
                <span key={item.id} className="inline-flex items-center gap-1.5 rounded-full bg-white/5 border border-white/10 px-3 py-1 text-xs text-white/60">
                  <span>{item.vendorLogo}</span>
                  <span>{item.vendor}</span>
                  <button onClick={() => removeItem(item.id)} className="text-white/30 hover:text-white/60 ml-1">
                    <X className="h-3 w-3" />
                  </button>
                </span>
              ))}
      ...
      {activeTab === 'ranking' && (
        <div className="space-y-4">
          <div className="flex items-center gap-3 flex-wrap">
            <span className="text-[10px] text-white/40">Rank by:</span>
            {([
            ...
            <select
              ...
              className="rounded-lg bg-white/[0.04] border border-white/10 text-xs text-white/60 px-2.5 py-1.5 focus:outline-none cursor-pointer"
            >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="w-full max-w-lg rounded-2xl bg-[#0f1a28] border border-white/20 p-5 space-y-4 shadow-2xl">
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-semibold text-white/90">Add to Compare</h3>
              <button onClick={() => setShowAddModal(false)} className="text-white/40 hover:text-white/60">
                <X className="h-5 w-5" />
              </button>
            </div>
            <p className="text-xs text-white/40">Select hardware to add ({compareItems.length}/4 selected)</p>
            <div className="space-y-2 max-h-80 overflow-y-auto">
              {availableItems.length === 0 ? (
                <p className="text-center py-8 text-white/30 text-xs">All available hardware is already in the comparison.</p>
              ) : (
                availableItems.map(item => (
                  <div key={item.id} className="flex items-center gap-3 rounded-xl bg-white/[0.04] border border-white/10 p-3 hover:bg-white/[0.07] transition-colors">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px] text-white/40">{item.modelName} · ${item.pricePerUnit}/{item.priceUnit}</p>
                    </div>
                    <button
