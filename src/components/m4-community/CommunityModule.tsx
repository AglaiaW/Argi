  ...
  return (
    <div className="absolute inset-y-0 right-0 z-20 flex flex-col border-l border-[rgba(255,255,255,0.08)] bg-[#0a1628] shadow-2xl w-[340px]">
        ...
        {item.tags && Array.isArray(item.tags) && (
          <div className="flex flex-wrap gap-2 mb-4">
            {(item.tags as string[]).map((tag) => (
              <span key={tag} className="rounded-full border border-[rgba(255,255,255,0.08)] bg-slate-100 px-2.5 py-1 text-[10px] text-slate-400">#{tag}</span>
            ))}
          </div>
        )}
      ...
      <div className="border-t border-[rgba(255,255,255,0.06)] p-4 space-y-2">
        ...
        <button onClick={() => { alert('已添加到收藏夹') }} className="flex w-full items-center justify-center gap-2 rounded-2xl border border-[rgba(255,255,255,0.08)] py-2.5 text-sm text-slate-400 transition-all">
    ...
    >
      ...
      <div className="flex-1 flex flex-col justify-around gap-2">
        {SECTIONS.map((s) => {
          const Icon = s.icon
          return (
            <button key={s.id} onClick={() => onSectionClick?.(s.id)} className="group flex items-center gap-3 rounded-xl border border-[rgba(255,255,255,0.04)] bg-slate-100 p-3 text-left transition-all hover:border-[rgba(255,255,255,0.1)] hover:bg-[#1a2744]">
...
function TalentBlock({ item, size, onClick }: { item: typeof TALENTS[0]; size: 'md' | 'sm'; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className="group flex flex-col items-center overflow-hidden rounded-2xl border border-[rgba(255,255,255,0.08)] bg-[#0a1628] p-3 text-center transition-all duration-300 hover:border-[rgba(255,255,255,0.12)] active:scale-[0.98]"
